...
class BITStar:
    def __init__(
        ...
        self.first_batch_factor = first_batch_factor
    ...
    def plan(
        ...
        while self.curr_batch < self.max_batch:
            ...
            if len(self.e_queue) == 0 and len(self.v_queue) == 0:
                ...
                if self.curr_batch > 0 and print_stats:
                    self._print_and_reset_times(batch_start_time)
                
                # prune
                self._prune(self._g(self.goal) * self.prune_threshold)
                
                # resample
                nb_samples = self.batch_size
                if self.curr_batch == 0:
                    nb_samples = int(self.batch_size * self.first_batch_factor)
            ...
            # if v_queue is empty, start a new batch
            if should_be_reset:
                self.e_queue = set()
                self.v_queue = set()
                continue
    ...
    def _sample(self, nb_samples: int, best_cost_to_goal: float) -> Set[Vertex]:
        ...
        if best_cost_to_goal == float("inf"):
            ...
            return self._sample_uniform(nb_samples)
...
def main():
    ...
    # bit_star = BITStar(
    #     map, start, goal, batch_size=30, max_batch=30, stop_on_goal=False, limit_secs=10
    # )
    # time_start = time.process_time()
